...
def build_and_solve(feed=70, start_recovery =0.5, **kwargs):
...
# added product-specific costing variables instead of summing them
    for p, product in m.fs.bpmed.flow_mass_product:
        ...
        m.fs.costing.add_annual_product_generation(
            m.fs.bpmed.flow_mass_product[p, product], "AnnualProduct_" + product
        )


    m.fs.product_objective = Objective(
        expr=m.fs.costing.LCOP_HCl + m.fs.costing.LCOP_NaOH
        + sum(
    ...
    # solving once BEFORE optimization (baseline solve)
    baseline_result = solver.solve(m, tee=False)
    if baseline_result.solver.termination_condition != TerminationCondition.optimal:
        print(f"Warning: baseline solve at feed={feed} resulted in {baseline_result.solver.termination_condition}")
...
# solving once at 0.5 (or should it be 0.7?)

    m.fs.bpmed.nacl_recovery.fix(start_recovery)

# extra solve, not needed
...
#    m.fs.bpmed.nacl_recovery.unfix()
    ...
    if result.solver.termination_condition != TerminationCondition.optimal:
        print(f"Warning: solve at feed={feed}, recovery={start_recovery} resulted in {result.solver.termination_condition}")
    ...
    print("LCOP_HCl:",
        value(m.fs.costing.LCOP_HCl))
    print("LCOP_NaOH:",
        value(m.fs.costing.LCOP_NaOH))
...
def build_sweep_params(m, num_samples=36):
    ...
    sweep_params["NaCl Recovery"] = LinearSample(
        m.fs.bpmed.nacl_recovery, 0.5, 1.0, num_samples
...
def custom_optimize(model):
    solver = get_solver()
    solver.options["max_iter"] = 7000  # Set your max iterations
...
if __name__ == "__main__":
    num_samples = 2
    # 36 points for 0.5-0.85, 52 points for 0.5 to 1.0
    ...
    #         build_model=partial(build_and_solve, feed=feed, start_recovery=0.5),
    ...
    #         csv_results_file_name=f"voltub800_check_costing_vs_recovery_{feed}gL_{timestamp}.csv",
    #     )
    results_array, results_dict = parameter_sweep(
        build_model=partial(build_and_solve, feed=70, start_recovery=0.5),
        build_sweep_params=build_sweep_params,
        build_sweep_params_kwargs={"num_samples": num_samples},
        build_outputs=build_outputs,
        optimize_function=custom_optimize,
        #csv_results_file_name=f"voltub1200_check_costing_vs_recovery_70gL_{timestamp}.csv",
        h5_results_file_name="test2sample_sweep_70gL_{timestamp}.h5"
    )

# write function to re-initialize and put this into param sweep
# check the nan cases