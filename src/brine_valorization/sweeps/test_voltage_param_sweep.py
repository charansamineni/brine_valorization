...
def build_sweep_params(m, num_samples=10):
    ...
    sweep_params = {}
    sweep_params["Voltage Applied"] = LinearSample(
        m.fs.bpmed.bpmed[0].voltage_applied, 150, 500, num_samples
...
def build_outputs(m):
    ...
    outputs["Dilute Feed Prop"]= m.fs.dilute_feed.feed.properties[0].flow_vol_phase["Liq"]

    return outputs


num_samples = 10
file_save = "voltage_parameter_sweep_1_Jul16.csv"
