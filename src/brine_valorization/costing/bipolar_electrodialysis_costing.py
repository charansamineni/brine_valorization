...
@register_costing_parameter_block(
    ...
    parameter_block_name="bipolar_electrodialysis_costing",
)
def cost_bipolar_electrodialysis(
    ...
    #     blk.costing_package.cost_flow(
    #         pyo.units.convert(blk.unit_model.acid_produced, to_units=pyo.units.kg * pyo.units.second ** -1, ),
    #         "acid_flow_cost")
    # if has_bp_base_value:
    #     blk.costing_package.cost_flow(
    #         pyo.units.convert(blk.unit_model.base_produced, to_units=pyo.units.kg * pyo.units.second ** -1, ),
    #         "base_flow_cost")
...
def cost_bipolar_electrodialysis_stack(blk):
    ...
    Assumes the unit_model has a `cell_triplet_num`, `cell_width`, and `cell_length`
    set of variables used to size the total membrane area.

    """
    if blk.find_component("capital_cost") is None:
